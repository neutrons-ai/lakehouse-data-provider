#!/usr/bin/env python3
"""
Ingest Parquet files into Template Provider Iceberg tables.

This script loads Parquet files into the template_provider lakehouse tables.
...
- *records*.parquet -> records table
- *events*.parquet -> events table

Usage:
    # Via spark-submit
    spark-submit ingest_data.py --input-dir /path/to/data --catalog nessie --namespace template_provider

    # Via lakehouse CLI (once integrated)
    lakehouse ingest /path/to/data --namespace template_provider --script ingest_data.py
"""
...
def get_target_table_from_metadata(file_path: str) -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            print(f"  Warning: pyarrow not available, cannot read metadata from {file_path}")
            return None
...
def ingest_files(
    ...
    df.writeTo(full_table).using("iceberg").createOrReplace() if mode == "overwrite" else df.writeTo(
        full_table
...
def main():
    parser = argparse.ArgumentParser(
        description="Ingest Parquet files into template provider Iceberg tables"
    )
    parser.add_argument("--input-dir", required=True, help="Input directory with Parquet files")
    parser.add_argument("--catalog", default="nessie", help="Catalog name (default: nessie)")
    parser.add_argument(
        "--namespace", default="template_provider", help="Namespace name (default: template_provider)"
    )
    # Keep --database as alias for backwards compatibility
    parser.add_argument("--database", dest="namespace", help="[DEPRECATED] Use --namespace instead")
    parser.add_argument(
        "--mode", choices=["append", "overwrite"], default="append", help="Write mode (default: append)"
    )
    parser.add_argument("--table", help="Specific table to ingest (skip auto-detection)")
    parser.add_argument("--dry-run", action="store_true", help="Show routing without ingesting")
    ...
    print(f"Template Provider Data Ingestion")
    print(f"{'=' * 50}")
    ...
    if args.table:
        ...
        total_rows = ingest_files(spark, files, args.table, args.catalog, args.namespace, args.mode)
    else:
        ...
        for table_name, files in sorted(file_mapping.items()):
            if files:
                try:
                    rows = ingest_files(
                        spark, files, table_name, args.catalog, args.namespace, args.mode
                    )
                    total_rows += rows
                    tables_ingested += 1
                except Exception as e:
